...
class CardViewModel2: ObservableObject {
    ...
    let CardData : [String : Array<String>] = [
        ...
        "sodastraw" : ["종유관","지하수가 석회동굴의 천장에 맺혀 생긴다., 이때 생긴 석회질이 천장에 수직으로 매달린 빨대 모양을 형성한 것이다."],
        "corrosionhole" : ["용식공","동굴 천장에 패인 구멍 형태로 발달된 지형으로,석회수가 석회암을 부식시키는 용식작용으로 인해 반원형의 용식공이 형성된다."],
        "transparent snail" : ["투명 달팽이","어두운 곳에 생육하는 특성때문에 다른 달팽이보다 눈이 짧다. , 그리고 투명한 등껍질을 가지고 있다. ,움직임이 거의 없고 이동시에는 흐르는 물을 타고 이동한다."],
        ...
        "stalactite" : ["종유석","동굴 천정에서 지하수가 물방울로 떨어질 때,지하수에 용해되어 있던 탄산칼슘 성분이,천정의 물방울이 떨어지는 지점에 집적되어,시간이 흐룰수록 동굴바닥쪽으로 성장해 가는 고드름 모양의 석회 기둥이다."]
    ]
